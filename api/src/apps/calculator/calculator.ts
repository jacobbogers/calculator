...
export default function evaluateExpression(data: RecordedActionsSansDigit[], ctx: ExpressionContext): number {
    ...
    while (ctx.position < data.length - 1) {
        ...
        if (isNaN(result)) {
            ctx.failureAt = ctx.position;
            ctx.reason = `Expression between ${ctx.position + 1} and ${subctx.position + 1} yielded a NaN`;
            return NaN;
        }
        leftValue = result;
