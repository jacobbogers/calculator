...
describe('app integration test', () => {
    ...
    describe('calculation integration tests', () => {
        it('5x2 returns 10', async () => {
            const response = await root.inject({
                method: 'POST',
                url,
                payload: [{ type: 'value', payload: 5 }, { type: 'multiply' }, { type: 'value', payload: 2 }]
            });
        ...
        it('6 x .2 returns 1.2', async () => {
            const response = await root.inject({
                method: 'POST',
                url,
                payload: [{ type: 'value', payload: 6 }, { type: 'multiply' }, { type: 'value', payload: 0.2 }]
            });
            expect(JSON.parse(response.body)).toEqual({
                type: 'value',
    ...
    describe('edge cases and errors', () => {
        ...
        it('force http-500 response: (non production) Unsupported media type in request', async () => {
            ...
            const errorBody: Record<string, Record<string, string>> = JSON.parse(response.body) as Record<
                string,
                Record<string, string>
            >;
            const errorMessage = errorBody?.error?.message;
            expect(errorMessage).toMatch(/internal server error: FastifyError: Unsupported Media Type/);
