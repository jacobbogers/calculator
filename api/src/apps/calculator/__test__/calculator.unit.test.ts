...
describe('calculator unit test', () => {
    describe('Evaluate Expressions', () => {
        it('6 x 0.2 = 1.2', () => {
            ...
            const result = evaluateExpression(
                [{ type: 'value', payload: 6 }, { type: 'multiply' }, { type: 'value', payload: 0.2 }],
                ctx
            );
        ...
        it('Expression is a value digit 1.42334E-23', () => {
            const ctx: ExpressionContext = { rank: 0, position: 0 };
            const result = evaluateExpression([{ type: 'value', payload: 1.42334e-23 }], ctx);
    ...
    describe('invalid expressions and edge cases', () => {
        ...
        it('dangling operator ( 4 x ..  = )', () => {
            const ctx: ExpressionContext = { rank: 0, position: 0 };
            const result = evaluateExpression([{ type: 'value', payload: 4 }, { type: 'multiply' }], ctx);
            expect(ctx).toEqual({
                failureAt: 1,
                position: 0,
                rank: 0,
        ...
        it('dangling operator in subexpression ( 4 + 6 * ..  = )', () => {
            const ctx: ExpressionContext = { rank: 0, position: 0 };
            const result = evaluateExpression(
                [{ type: 'value', payload: 4 }, { type: 'multiply' }, { type: 'value', payload: 6 }, { type: 'add' }],
                ctx
            );
            expect(ctx).toEqual({
                failureAt: 3,
        ...
        it('expression must start with a value ( x 6  = )', () => {
            const ctx: ExpressionContext = { rank: 0, position: 0 };
            const result = evaluateExpression([{ type: 'multiply' }, { type: 'value', payload: 4 }], ctx);
            expect(ctx).toEqual({
                failureAt: 0,
                position: 0,
                rank: 0,
        ...
        it('Division by zero (  1 / 0 = Infinity ) and (-1/0 = Infinity)', () => {
            {
                const ctx: ExpressionContext = { rank: 0, position: 0 };
                const result = evaluateExpression(
                    [{ type: 'value', payload: 1 }, { type: 'divide' }, { type: 'value', payload: 0 }],
                    ctx
                );
                expect(result).toBe(Infinity);
        ...
        it('valid expression syntax 0/0, result is a NaN', () => {
            const ctx: ExpressionContext = { rank: 0, position: 0 };
            const result = evaluateExpression(
                [{ type: 'value', payload: 0 }, { type: 'divide' }, { type: 'value', payload: 0 }],
                ctx
            );
            expect(result).toBeNaN();
            expect(ctx).toEqual({
