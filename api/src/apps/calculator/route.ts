import type { RouteOptions, RawServerDefault, RawRequestDefaultExpression, RawReplyDefaultExpression } from 'fastify';
import StatusCodes from 'http-status-codes';

import type { RecordedActionsSansDigit, ServerResponseError, ValueAction } from './shared/types';
...
import { ERR_APP_CALCULATOR_REJECT, API_REST_PATH_SUFFIX } from './shared/constants';
...
const calculatorRoute: RouteOptions<
    ...
    Required<{
        ...
        Reply: ServerResponseError | ValueAction;
        Headers: {
...
> = {
    ...
    handler: async function (req, reply): Promise<void> {
        ...
        //
        if (!isNaN(result)) {
            reply.code(StatusCodes.OK).send({ type: 'value', payload: result });
            return;
        }
        // invalid request
        const errorObj = {
            error: {
                ...
                message: ctx.reason as string, // it is never undefined here
                reqId: String(req.id)
            }
        };
